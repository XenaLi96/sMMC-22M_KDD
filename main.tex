...
Reviewer and benchmark packages are derived artifacts and do not introduce
additional biological samples. Anonymous GitHub:
\protect\url{https://anonymous.4open.science/r/sMMC-22M-DB75}}
...
53,989 &
Native cell &
In-domain and context-aware evaluation &
\makecell[l]{\texttt{dbic\_seq/}\\\texttt{*.h5ad}} \\
...
fixed &
Split-indexed &
In-domain benchmark for all categories; multi-sample and platform-transfer
...
{\footnotesize
\emph{Notes:}
The 99 unique samples correspond to 100 versioned records. ``sub.'' and
``rec.'' indicate subset-level and record-level derived artifacts,
respectively. The platform rows sum to the expanded working manifest; exact
provenance relative to earlier public resources is reported separately in
\Cref{tab:main_provenance_audit}.
\par}
\end{table*}
...
\caption{\textbf{Provenance audit for the expanded \dataset working manifest.}
Categories are mutually exclusive with respect to HEST-1K v1.1.0 and the
2025-02-12 STimage-1K4M snapshot. Counts are aligned target cells after QC.
Anonymous GitHub: \protect\url{https://anonymous.4open.science/r/sMMC-22M-DB75}}
\label{tab:main_provenance_audit}
...
\vspace{2pt}
{\footnotesize\emph{Notes:} The in-house acquisition measured 54{,}304 cells;
QC removed 315, leaving 53{,}989 (0.1907\% of the current manifest). One
versioned breast slide explains 99 unique samples versus 100 records. Visium HD
profiles are derived bin-to-cell targets; Xenium and DBiC-seq provide native
cell-aligned records.}
\end{table*}

Gene targets are omitted from the two main manifest tables for readability: Xenium uses panel genes, Visium HD uses processed full-transcriptome targets, and cross-platform experiments use harmonized shared genes. Full per-sample gene counts, source identifiers, QC status, benchmark-coverage fields, and checksum-backed file paths are reported in the appendix manifest and released CSV.

The compact manifest in \Cref{tab:main_release_manifest} preserves the
benchmark-package overview, while \Cref{tab:main_provenance_audit} makes the
new-versus-aggregated scale claim directly auditable. The complete per-sample
manifest remains in the appendix and release package, where each source sample
is linked to its platform, organ, cell count, gene target, upstream source,
license note, inclusion status, and QC outcome.
...
buffer between train and test cells. \Cref{tab:breadth_25a,tab:breadth_25b}
report the full category-level results in two readable parts. Image features
...
cases rather than hidden in a pooled average. The final row of
\Cref{tab:breadth_25b} separately summarizes 30 native-Xenium samples: image
...
\caption{\textbf{Per-category cell-aligned breadth benchmark (Part I).}
Results use top-50 training-selected HVGs, four contiguous spatial holdouts,
and a 5\% train--test buffer. Bold Gene-Pearson entries mark the best of image,
coordinate-only, and spatial KNN.}
...
Release category (species) & Image & Coordinate & Spatial KNN &
Gene Spearman & Cell Pearson & F1 \\
\midrule
Bone (mouse) & 0.030 & \textbf{0.124} & 0.055 & 0.037 & 0.180 & 0.240 \\
...
Colon (mouse) & \textbf{0.615} & -0.137 & 0.072 & 0.588 & 0.739 & 0.744 \\
Colorectal (human) & \textbf{0.431} & 0.056 & 0.083 & 0.419 & 0.532 & 0.623 \\
Embryo (mouse) & \textbf{0.278} & 0.037 & 0.045 & 0.251 & 0.517 & 0.342 \\
...
\setlength{\tabcolsep}{5.0pt}
\renewcommand{\arraystretch}{1.10}
\caption{\textbf{Per-category cell-aligned breadth benchmark (Part II).}
The 30-sample macro is the native-Xenium summary and is not a
cell-weighted average over the category rows.}
...
Small Intestine (mouse) & \textbf{0.572} & -0.104 & 0.067 & 0.548 & 0.701 & 0.715 \\
Tonsil (human) & \textbf{0.294} & 0.028 & 0.018 & 0.249 & 0.462 & 0.399 \\
Xenograft (human + mouse) & \textbf{0.387} & 0.041 & 0.049 & 0.362 & 0.526 & 0.589 \\
\midrule
\textbf{30-sample native-Xenium macro} & \textbf{0.324} & 0.046 & 0.053 &
...
\noindent\textbf{Independent condition-shift support.}
Because age is nested within the ovarian samples and the panels differ, the
ovarian result should be interpreted as an age-associated,
sample-confounded case study rather than a causal age effect. We therefore add
a separate native-Xenium skin experiment using the same spatially buffered
protocol. Replication between two non-diseased skin sections reaches Gene
Pearson 0.359, whereas normal/melanoma transfers reach only 0.063--0.094
(\Cref{tab:main_skin_context}). This second experiment preserves the original
context argument while showing that the observed collapse is not specific to
the ovarian example.
...
frozen 22M release snapshot; the updated provenance totals are reported in
Table~\ref{tab:main_release_manifest}. In the current appendix we focus on
representative benchmark subsets analyzed under the same three standardized
regimes used throughout the paper: in-domain, cross-patient, and
cross-platform.
