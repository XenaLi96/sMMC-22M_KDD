...
\noindent\textbf{Companion foundation-model audit.}
To test whether this pattern extends beyond one spatial task, we applied the
same support-aware reporting principle in a companion audit of external
CELLxGENE and TCGA representations. The audit covers Geneformer,
...
in \Cref{tab:context_bias_external} show that high average balanced accuracy
...
\caption{\textbf{Representative context-bin failures in a companion external
foundation-model audit.} Values are balanced accuracy; higher Average/Worst
and lower Gap are better. These CELLxGENE/TCGA rows validate the audit
protocol and are not \dataset benchmark scores.}
\label{tab:context_bias_external}
...
Across all 50 support-eligible foundation-model rows in that campaign, 43
...
held-out-context evaluation in the companion campaign, label--context
...
This framing also changes how benchmark results should be read. A high in-domain score is evidence of local interpolation, not a general solution. A strong cross-sample score is more informative, but still does not guarantee robustness to shifts in age, platform, disease composition, or tissue processing. The skin transfer, companion foundation-model audit, and ovarian sample-confounded case collectively illustrate why future ST benchmarks need explicit context fields and support matrices rather than only larger image--expression tables. Context-aware fine-tuning is a benchmarkable response to these failures, not a guarantee that every unsupported shift can be corrected.
...
The original sample aliases in \Cref{fig:ovarian_age_effect} denote a 60+
training sample (AK), a 60+ target sample (AD), and a younger-than-40 target
sample (AL). Age is nested within sample and the molecular panels differ, so
this comparison is reported as a sample-confounded stress test rather than a
causal age-effect estimate.

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\linewidth]{age.png}
\caption{\textbf{Ovarian age-associated, sample-confounded transfer.}
Left: accuracy and rank-correlation metrics drop under transfer from the 60+
training sample to the younger-than-40 target. Middle: the model predicts far
fewer active genes on that target, creating a sparsity failure. Right: global
distribution distances do not reliably diagnose the performance collapse.
AK, AD, and AL denote the 60+ training, 60+ target, and younger-than-40 target
