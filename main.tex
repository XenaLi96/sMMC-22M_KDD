...
We next tested whether the scale claim extends beyond the two tissues used for
encoder scaling. Genes were selected from the training partition only, and
every category used the same spatially buffered evaluation design.
Image features led Gene Pearson in 21 of 25 categories under training-only gene
selection, four contiguous spatial edge holdouts, and a 5\% coordinate-span
buffer (\Cref{tab:breadth_25}). Bone, Liver, Plant, and Seed remained explicit
failure cases. Across 30 native-Xenium samples, image Gene Pearson reached
0.324 versus 0.046 for coordinate-only and 0.053 for spatial KNN
...
(\Cref{tab:main_release_manifest}). Large assets are linked through checksum-
backed entries and documented download paths. The repository also includes
license notes, responsible-use terms, and fixed split definitions so that
future models can be evaluated without redefining preprocessing or held-out
partitions. No benchmark score in this paper depends on hidden internal files.
